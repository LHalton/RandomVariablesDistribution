...
shinyServer(function(input,output){
  dat <- reactive({
    dist <- switch(input$dist,
                   norm=rnorm, unif=runif, t=rt2, F=rf, exp=rexp2, chisq=rchisq2, lnorm=rlnorm)
    
    def.args <- switch(input$dist,
                       norm=c(input$mean,input$sd), unif=c(input$min,input$max), t=c(input$dft), F=c(input$df1,input$df2),
                       exp=c(input$rate2), chisq=c(input$dfx), lnorm=c(input$meanlog,input$sdlog))
  ...
  output$dist1 <- renderUI({
    lab <- switch(input$dist,
                  norm="Mean:", unif="Minimum:", t="Degrees of freedom:", F="Numerator degrees of freedom:", exp="Rate:",
                  chisq="Degrees of freedom:", lnorm="Mean(log):")
    ini <- switch(input$dist,
                  norm=0, unif=0, t=15, F=1, exp=1, chisq=1, lnorm=0)
    numericInput(dat()[[2]][1],lab,ini)
  })
  
  output$dist2 <- renderUI({
    lab <- switch(input$dist,
                  norm="Standard deviation:", unif="Maximum:", F="Denominator degrees of freedom:", lnorm="Standard deviation(log)")
    ini <- switch(input$dist,
                  norm=1, unif=1, F=15, gam=1, lnorm=1, beta=2)
    if(any(input$dist==c("norm","unif","F","lnorm"))) numericInput(dat()[[2]][2],lab,ini)
  })
